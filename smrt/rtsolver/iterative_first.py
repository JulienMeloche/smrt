...
from ..core.fresnel import snell_angle, brewster_angle
...
class IterativeFirst(object):
    ...
    def solve(self, snowpack, emmodels, sensor, atmosphere=None):
        ...
        I = self.calc_intensity(snowpack, emmodels, sensor, snowpack.interfaces, substrate, effective_permittivity, substrate_permittivity, mu0)
    ...
    def calc_intensity(self, snowpack, emmodels, sensor, interfaces, substrate, effective_permittivity, substrate_permittivity, mu0):
        ...
        dense_factor_0 = np.atleast_3d((1 / effective_permittivity[0].real) * (mus[-1] / mus[0])).reshape(len_mu, 1, 1)
        ...
        for l in range(nlayer):
            ...
            mu_l_reshape = np.atleast_3d(mus[l]).reshape(len(mu0), 1, 1)
            gammas = np.exp(- layer_optical_depth / mu_l_reshape)
            gammas2 = gammas**2
            ...
            I1_2B = (
                Ttop_coh_m
                @ (thickness[l] * gammas2 / mu_l_reshape * (P_Bi_Down @ Rbottom_coh_m + Rbottom_coh_m @ P_Bi_Up))
                @ I_l
            )
            ...
            if l < nlayer - 1:
                mu_l1_reshape = np.atleast_3d(mus[l + 1]).reshape(len(mu0), 1, 1)
                # dense snow factor (I think) eq 22a and eq 22b in Tsang et al 2007
                dense_factor_l = (effective_permittivity[l].real/ effective_permittivity[l+1].real) * (mu_l_reshape / mu_l1_reshape)
