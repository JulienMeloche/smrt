...
class IterativeFirst(object):
    ...
    def solve(self, snowpack, emmodels, sensor, atmosphere=None):
        ...
        self.effective_permittivity = [
            emmodel.effective_permittivity() for emmodel in emmodels]

        self.substrate = snowpack.substrate

        if snowpack.substrate is not None:
            self.substrate_permittivity = self.substrate.permittivity(
            sensor.frequency)
        ...
        other_data = {
            "effective_permittivity": xr.DataArray(
                self.effective_permittivity, coords=[layer_index]
            ),
        ...
        if self.return_contributions:
            ...
            return make_result(sensor, intensity, 
                            coords=[('contribution', ['total', 'direct_backscatter', 'reflection_backscatter', 'double_bounce'])] + coords, 
    ...
    def calc_intensity(self, snowpack, emmodels):
        ...
        interface_l = zInterfaceProperties(self.sensor.frequency, self.interfaces, self.substrate,
                                          self.effective_permittivity,  self.mu0, npol, self.nlayer, dphi)
        ...
        for l in range(nlayer):
            ...
            if scat_albedo > 0.3:
                smrt_warn(f'Warning : scattering albedo ({np.round(scat_albedo,2)}) might be too high' 
            ...
            phases = np.array([emmodels[l].phase(np.array([mu, -mu]).squeeze(), np.array([mu, -mu]).squeeze(), 
                                np.pi, npol).values.squeeze() for mu in mus[l]])
            ...
            # (-mu, mu) : [1,0]
            # (mu, -mu) : [0,1]
            P_Up = np.array([[phases[i][tuple(index)][1,0] for index in index_comb] for i in range(len_mu)]).reshape(len_mu, npol, npol)
            P_Down = np.array([[phases[i][tuple(index)][0,1] for index in index_comb]for i in range(len_mu)]).reshape(len_mu, npol, npol)
            # and up and down  bi-static scatter
            # (mu, mu) : [0,0]
            # (-mu, -mu) : [1,1]
            P_Bi_Up = np.array([[phases[i][tuple(index)][0,0] for index in index_comb ] for i in range(len_mu)]).reshape(len_mu, npol, npol)
            P_Bi_Down = np.array([[phases[i][tuple(index)][1,1] for index in index_comb] for i in range(len_mu)]).reshape(len_mu, npol, npol)
        ...
        if self.substrate is None and optical_depth < 5:
            smrt_warn("The solver has detected that the snowpack is optically shallow (tau=%g) and no substrate has been set, meaning that the space "
...
class zInterfaceProperties(object):
    ...
    def __init__(self, frequency, interfaces, substrate, permittivity, mu0, npol, nlayer, dphi):
        ...
        self.Rbottom_diff[-1] = interfaces[0].diffuse_reflection_matrix(frequency,
                                                                        1,
                                                                        permittivity[0],
                                                                        mu0,
                                                                        mu0, dphi, npol) \
            if hasattr(interfaces[0], "diffuse_reflection_matrix") else smrt_matrix(0)
        ...
        for l in range(nlayer):
            ...
            self.Rtop_diff[l] = interfaces[l].diffuse_reflection_matrix(frequency,
                                                                        eps_l,
                                                                        eps_lm1,
                                                                        self.mu[l],
                                                                        self.mu[l],
                                                                        dphi,
                                                                        npol) \
                if hasattr(interfaces[l], "diffuse_reflection_matrix") else smrt_matrix(0)
            ...
            if l < nlayer - 1:
                ...
                self.Rbottom_diff[l] = interfaces[l+1].diffuse_reflection_matrix(frequency,
                                                                                 eps_l,
                                                                                 eps_lp1,
                                                                                 self.mu[l],
                                                                                 self.mu[l],
                ...
                self.Rbottom_diff[l] = substrate.diffuse_reflection_matrix(frequency,
                                                                           eps_l,
                                                                           self.mu[l],
                                                                           self.mu[l],
                                                                           dphi,
                                                                           npol) \
                    if hasattr(substrate, "diffuse_reflection_matrix") else smrt_matrix(0)
